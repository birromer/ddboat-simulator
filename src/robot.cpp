...
void cmd_r_Callback(const geometry_msgs::Twist::ConstPtr& msg) {
  ROS_INFO("received command right: [%f]", msg->data);
  cmd_r = msg->data;
}

void cmd_l_Callback(const geometry_msgs::Twist::ConstPtr& msg) {
  ROS_INFO("received command left: [%f]", msg->data);
  cmd_l = msg->data;
}

void integration_euler(double &x1, double &x2, double &x3, double &x4, double u1, double dt) {
  x1 = x1 + dt * (x4*cos(x3));
  x2 = x2 + dt * (x4*sin(x3));
  x3 = x3 + dt * (P1*(u1-u2));
...
int main(int argc, char **argv) {
  ...
  ros::Subscriber z_sub = n.subscribe("z_u1", 1000, z_Callback);
  ...
  while (ros::ok()) {
    u1 = z;
    integration_euler(x, y, theta, u1, dt);
    ...
    tf::Quaternion q;
    q.setRPY(0, 0, theta);  // roll, pitch, yaw - theta normally pitch, but i think it means yaw now
